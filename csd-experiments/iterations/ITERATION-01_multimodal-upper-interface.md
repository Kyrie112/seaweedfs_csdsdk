...
V1 只改造上层调用与跨 chunk 编排,单个 chunk 的底层执行仍与 V0 相同:

```text
每个 chunk:
  filer 扇出 → volume ReadVolumeNeedle
  → n.Data(主机内存)
  → 临时文件
  → 算子脚本读取/计算
  → 结果回 filer
...
filer 汇总 N 个 chunk:
  每个 chunk 返回小体积数值 → big.Int 相加 → 返回用户
```
...
| 缺陷 | V1 状态 |
| --- | --- |
| 只能单 chunk | 已解决:跨 chunk 并发扇出 + filer 聚合 |
| 只有文件 HTTP 一种触发 | 已解决:S3/块/统一 API |
| 非数值结果跨 chunk | 显式 400,不静默给错结果 |

下层仍保留的 V0 缺陷(V2 负责解决):

| 缺陷 | 说明 |
| --- | --- |
| 主机内存全量读入 | 每个 chunk 仍先进 `n.Data` |
| 临时文件 | 每个 chunk 仍写临时文件 |
| CPU 脚本 | 计算仍发生在通用 CPU |
| 多次拷贝 | 磁盘 → 内存 → 临时文件 → 脚本 |

结论:V1 让用户层可用,但没有改善单 chunk 的字节流动。
